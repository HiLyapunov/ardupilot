...
Vector3f AC_PDNN_SO3::update_all(const Matrix3f &R_c, const Matrix3f &R, const Vector3f &Omega, float dt, bool Rc_active)
{
   ...
   if (_Rc_active) {
    ...
    _pdnn_output.x = 0.01f * (-_e_R.x * 100.0f - _e_Omega.x * 40.0f - 1.0f * _integrator.x - _geomrtry_output.x - 0.0f *_phi_x + 0.0f*Aug.x); 
    _pdnn_output.y = 0.01f * (-_e_R.y * 100.0f - _e_Omega.y * 40.0f - 1.0f *_integrator.y - _geomrtry_output.y- 0.0f * _phi_y + 0.0f*Aug.y);
    _pdnn_output.z = 0.02f * (-_e_R.z * 100.0f - _e_Omega.z * 40.0f - 1.0f *_integrator.z - _geomrtry_output.z - 0.0f *_phi_z + 0.0f*Aug.z); //偏航误差e_R.z很容易就趋近于0，会导致无法满足持续激励假设
   }
