...
void AC_AttitudeControl_Multi::rate_controller_run_dt(const Vector3f& gyro, float dt) //姿态速率控制
{
    ...
    _motors.set_roll(Md.x/3.5f);  //发送控制力矩给Mixer 13.3
    _motors.set_roll_ff(0.0f); //设置为无前馈
    _motors.set_pitch(Md.y/3.5f);
    _motors.set_pitch_ff(0.0f);
    _motors.set_yaw(Md.z/3.5f); //因为Mixer是基于NED配置，所以这里如果是用到NEU，需要再将NEU下的力矩翻转到NED发送给mixer
